#ifndef __ASSYSTEMBUS_H__
#define __ASSYSTEMBUS_H__
...
SC_MODULE(asSystemBus)
{
  public:
    ...
    typedef tlm_utils::simple_target_socket_tagged<asSystemBus>    slaveSock_t;
    typedef tlm_utils::simple_initiator_socket_tagged<asSystemBus> masteSock_t;
    // Sockets
    slaveSock_t    slaveSocket[nrMASTER];
    masteSock_t    masterSocket[nrSLAVE];
    /*-------------------------------------------------------------------------
    -- Constructor
    -------------------------------------------------------------------------*/
    SC_CTOR(asSystemBus);
  private:
				       ...
				       sc_dt::uint64& masked_address)
    {
      ...
      masked_address = address & BUSDECODEMASK;
      return target_nr;
    }
