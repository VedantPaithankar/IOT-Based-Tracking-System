#include "asSystemBus.h"

/*-----------------------------------------------------------------------------
-- Constructor
-----------------------------------------------------------------------------*/
template <unsigned int nrMASTER, unsigned int nrSLAVE>
asSystemBus<nrMASTER, nrSLAVE>::asSystemBus(sc_module_name nm)
  : sc_module(nm)
{
  //SC_HAS_PROCESS(asSystemBus);
  cout << left << setw(30) << "ConstructorCPP - asSystemBus: "
       << left << setw(36) << name()
  ...
  for (unsigned int i = 0; i < nrMASTER; ++i)
  {
    ...
    //slaveSocket[i] = new tlm_utils::simple_target_socket_tagged<asSystemBus>(txt);
    slaveSocket[i].register_b_transport(this,
		   &asSystemBus::b_transport,
		   i);
  }
...
-----------------------------------------------------------------------------*/
template <unsigned int nrMASTER, unsigned int nrSLAVE>
void asSystemBus<nrMASTER, nrSLAVE>::b_transport(int id,
					 payLoad_t& trans,
					 sc_time& t)
{
...
template <unsigned int nrMASTER, unsigned int nrSLAVE>
void asSystemBus<nrMASTER, nrSLAVE>::print01(char* text, unsigned int addr,
		 unsigned int maddr,
